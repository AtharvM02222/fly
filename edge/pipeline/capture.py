"""Video capture module with frame sampling."""
...
class CameraCapture:
    """
    Video capture with frame sampling.
    ...
    Decouples capture FPS from processing by sampling every Nth frame.
    """
    ...
    def read_frame(self) -> Optional[np.ndarray]:
        ...
        Returns:
            BGR frame (HxWx3 uint8) or None if failed
        """
        if not self.cap or not self.cap.isOpened():
            return None
    ...
    def sample_frames(self) -> Iterator[tuple[int, np.ndarray]]:
        """
        Yield sampled frames (every Nth frame).
